...
int main() {
	...
	Shader lighting_shader("C:\\Users\\Grant\\LearnOpenGL Files\\HDR\\lighting.vs", "C:\\Users\\Grant\\LearnOpenGL Files\\HDR\\lighting.fs");
	Shader quad_shader("C:\\Users\\Grant\\LearnOpenGL Files\\HDR\\quad.vs", "C:\\Users\\Grant\\LearnOpenGL Files\\HDR\\quad.fs");
	Shader lamp_shader("C:\\Users\\Grant\\LearnOpenGL FIles\\HDR\\lamp.vs", "C:\\Users\\Grant\\LearnOpenGL FIles\\HDR\\lamp.fs");

	Texture wood_texture(GL_TEXTURE_2D, Texture::DIFFUSE);
	wood_texture.bind();
	wood_texture.generate_texture("C:\\Users\\Grant\\LearnOpenGL Files\\HDR\\wood.png", GL_RGB);
	...
	light_positions.push_back(glm::vec3(0.0f, 0.0f, 49.5f)); // back light
	light_positions.push_back(glm::vec3(-1.4f, -1.9f, 9.0f));
	light_positions.push_back(glm::vec3(0.0f, -1.8f, 4.0f));
	light_positions.push_back(glm::vec3(0.8f, -1.7f, 6.0f));
	// colors
	...
	while (!glfwWindowShouldClose(window)) {
		...
		proj = glm::perspective(Camera::fov, (float)WindowInfo::width / WindowInfo::height, Camera::near_plane, Camera::far_plane);
		...
		for (unsigned int i = 0; i < light_colors.size(); i++) {
		...
		lighting_shader.set_bool("inverseNormals", true);
		render_tunnel();
		lighting_shader.set_bool("inverseNormals", false);
		...
		for (unsigned int i = 0; i < light_colors.size(); i++) {
			...
			lamp_shader.set_vec3("color", light_colors.at(i));
		...
		quad_shader.set_bool("hdrEnabled", true);
