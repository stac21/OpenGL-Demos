...
int main() {
	...
	// planet data
	Model planet_model("../Space Scene/Resources/Models/Planets/Temp Planet/planet.obj");
	glm::vec3 planet_pos = glm::vec3(-40.0f, 3.0f, 0.0f);
	glm::vec3 planet_scale = glm::vec3(7.0f);
	// ship data
	Model ship_model("../Space Scene/Resources/Models/Ships/E45/E 45 Aircraft_obj.obj");
	glm::vec3 ship_pos = glm::vec3(40.0f, 3.0f, 0.0f);
	glm::vec3 ship_scale = glm::vec3(0.5f);
	glm::vec3 ship_rotation_axis = glm::vec3(1.0f, 3.0f, 2.0f);
	
	// asteroid data
	Model asteroid_model("../Space Scene/Resources/Models/Asteroids/rock.obj");
	constexpr unsigned int ASTEROID_AMOUNT = 100;
	constexpr float RADIUS = 15.0f;
	constexpr float OFFSET = 2.5f;
	glm::mat4 asteroid_instanced_matrices[ASTEROID_AMOUNT];
	srand(glfwGetTime());
	...
	for (unsigned int i = 0; i < ASTEROID_AMOUNT; i++) {
		...
		// 4. add to array
		asteroid_instanced_matrices[i] = model;
	}
	...
	// TODO: delete
	Model asteroid_model3("../Space Scene/Resources/Models/Asteroids/rock.obj");

	StaticEntity planet(planet_pos, planet_scale, planet_model);
	StaticEntity asteroid(planet_pos, glm::vec3(3.0f), asteroid_model3);		// TODO: delete
	MoveableEntity ship(ship_pos, ship_model);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		asteroid.draw(model_shader);

		// draw moveable entities
		ship.update(model_shader, ship.get_x_pos(), ship.get_y_pos(), ship.get_z_pos(), ship_scale, ship_rotation_axis, angle);

		// asteroid belt
		instanced_model_shader.use();
		instanced_model_shader.set_mat4("projection", projection);
		glUniformMatrix4fv(glGetUniformLocation(model_shader.ID, "view"), 1, GL_FALSE, &view[0][0]);

		asteroid_model.draw_instanced(instanced_model_shader, asteroid_instanced_matrices, ASTEROID_AMOUNT);
